...
def write_sample_config():
    ...
    config.set('app-settings', 'font-size', 42)
    config.set('app-settings', 'font-colour', (1, 1, 1))
    config.set('app-settings', 'output_format', 'PDF')
    config.set('app-settings', 'attachments', 'attachments')
    config.add_section('user-settings')
    config.set('user-settings', 'xy_cordinates', (128, 128))
    ...
    config.set('email-sender', 'subject', 'Thank you for participate to make this world a better living place')
